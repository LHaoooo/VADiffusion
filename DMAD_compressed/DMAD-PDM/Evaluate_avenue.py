...
def MNADEval(model=None):
    ...
    parser.add_argument('--dataset_path', type=str, default='/home/Avenue', help='directory of data')
    ...
    for k in range(len(video_list)):
        m=[0 for i in range((METADATA['avenue']["testing_frames_cnt"])[k])]
    ...
    # load gt labels
    gt = pickle.load(
        open('/home/VADiffusion/data/avenue/ground_truth_demo/gt_label.json', "rb"))
    ...
    frames_idx = 0
    for cur_video_id in range(METADATA['avenue']["testing_video_num"]):
        cur_video_len = METADATA['avenue']["testing_frames_cnt"][cur_video_id]
