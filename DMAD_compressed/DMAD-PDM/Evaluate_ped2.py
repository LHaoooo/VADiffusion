...
def MNADEval(model=None):
    ...
    parser.add_argument('--dataset_path', type=str, default='/home/UCSD_ped2', help='directory of data')
    ...
    testset_yuvroot=os.path.join(args.dataset_path, "test_recyuv400/")
    ...
    for k in range(len(video_list)):
        m=[0 for i in range((METADATA['UCSD_ped2']["testing_frames_cnt"])[k])]
    ...
    with torch.no_grad():
        for k,(imgs, pred_frame_test, v_name) in enumerate(test_batch):
        ...
        # load gt labels
        gt = pickle.load(
            open('/home/VADiffusion/data/UCSD_ped2/ground_truth_demo/gt_label.json', "rb"))
        ...
        frames_idx = 0
        for cur_video_id in range(METADATA['UCSD_ped2']["testing_video_num"]):
            cur_video_len = METADATA['UCSD_ped2']["testing_frames_cnt"][cur_video_id]
