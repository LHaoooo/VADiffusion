...
def MNADTrain():
    ...
    trainset_yuvroot=os.path.join(args.dataset_path, "train_recyuv/")
    ...
    train_dataset = VideoDataset(args.c, trainset_yuvroot) 
    train_batch = data.DataLoader(dataset=train_dataset, batch_size=args.batch_size, 
                                num_workers=args.num_workers, shuffle=True, drop_last=True, pin_memory=args.pin_memory)
    # Loading dataset
    ...
    for epoch in range(args.epochs):
        model.train()
        for j,(imgs, pred_frame_idx,_) in enumerate(train_batch):
        ...
        #if epoch==0: optimizer.param_groups[-1]['lr'] = args.lr*100
        #if epoch>0:optimizer.param_groups[-1]['lr'] = args.lr*10
        print('----------------------------------------')
        print('Epoch:', epoch+1)
        print('----------------------------------------')
