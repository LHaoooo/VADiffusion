...
class convAE(torch.nn.Module):
    ...
    def forward(self, x, test=False):  # idx
        ...
        z_q_ = z_q_ + (1 - mask1) * self.bkg.detach()  # [idx]
        z_q_t = z_q_t + (1 - mask2) * self.bkg.detach() # [idx]

        return z_q_t, z_q_, z_q, z_e, commit_loss, vq_loss, offset1, offset2, mask, mask1, mask2

    def loss_function(self, x,  recon_x, z_q_, z_q, z_e, commit_loss, vq_loss, offset1=None, offset2=None, mask=None, mask1=None, mask2=None, compute_err=False): # idx
        weit = 1+100*torch.abs(F.avg_pool2d((x-self.bkg.detach()).abs()**0.5, kernel_size=15,stride=1, padding=7))   # bkg[idx]  
        ...
        if compute_err:
            ...
            self.mask_loss = self.mask_loss * 0.1 + ((1 - mask2.detach())*F.mse_loss(self.bkg,x, reduction='none')).sum() / (1 - mask2.detach()).sum()  # [idx]
