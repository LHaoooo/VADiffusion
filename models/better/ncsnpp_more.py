...
class NCSNpp(nn.Module):
  ...
  def __init__(self, config):
    super().__init__()
    self.config = config
    self.act = act = get_act(config)  # SiLU 激活函数
    ...
    self.nf = nf = config.model.ngf # We must prevent problems by multiplying by n_frames
    self.numf = numf = config.model.ngf # We must prevent problems by multiplying by n_frames
    ...
    if resblock_type == 'ddpm':
      ...
      # 创建一个新的函数，这个函数是在原来函数的基础上，固定了一些参数（也就是偏函数）
