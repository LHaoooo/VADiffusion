...
if [ "$#" -ne 2 ]; then
    echo "Usage: ./yuvtobin.sh [input dir] [output dir1] [output dir2]"
fi
...
do
    ...
    echo "${outname_yuv}"
    echo "${outname_bin}"
    ./TAppEncoderStatic -i ${inname} -c ../cfg/encoder_lowdelay_P_main.cfg -c ../cfg/per-sequence/Aio.cfg -b ${outname_bin} -o ${outname_yuv}
